...
void build_seg(int idx, int i, int j, int k)
{
	...
	st[k][idx] = (st[k][esq(idx)]|st[k][dir(idx)]);
}
...
ll query_seg(int idx, int i, int j, int l, int r, int k)
{
	if (i > r || j < l) return 0;
	if (i >= l && j <= r) return st[k][idx];
 
	int mid = ((i+j)>>1);
	ll x = query_seg(esq(idx), i, mid, l, r, k);
	ll y = query_seg(dir(idx), mid+1, j, l, r, k); 
	return (x|y);
}
...
ll query(int a, int b)
{
	...
	return (query_seg(1, 1, tam[h[a]], 1, pos[a]-pos[h[a]]+1, h[a])|query(pai[h[a]], b));
}
...
int main()
{
	...
	while (m--)
	{
		...
		else printf("%i\n", __builtin_popcountll(query(a, b)));
	}
}
